...
>(({ children, className, ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn(
        "group relative w-full h-28 cursor-pointer overflow-hidden border p-4 px-8 text-center text-2xl font-bold",
        className
      )}
      {...props}
    >
      <div className="flex items-center gap-4">
        ...
        <span className="font-pixelify text-left transition-all duration-300 group-hover:translate-x-16 group-hover:opacity-0">
          {children}
        </span>
      </div>
